...
def parameter_vis():
    algo_list = ['fgsm']
    # 'gaussian_noise', 'gaussian_noise_sign','gaussian_noise_std', 'gaussian_noise_sign_std']
    eta_list = [0.01]
    show = True
    mask_modes = {
        'positive': [None],
...
def parameter_total():
    ...
    if size == 1000:
        ...
        dataset_file = './data_stage2/images_100_0911.pth'
        save_result_file = 'one_step_attack_channel_topr_1016.xlsx'
    return algo_list, eta_list, mask_modes, model_list, data_root, dataset_file, save_result_file, show

def main():
    results = pd.DataFrame(columns=['model', 'algo', 'mask_mode', 'parameter', 'eta', 'pixel_attacked', 'attack_ratio_per_channel', 'l1_norm', 'l2_norm','success_rate', 'original_loss', 'attack_loss', 'pred_loss', 'run_time', 'batch_idx', 'batch_pictures'])
    ...
    for model_str in tqdm(model_list, desc="Models"):
        ...
        for batch_idx, (images, labels) in enumerate(tqdm(dataloader, desc="Batches", leave=False), 1): # 1表示从1开始计数
            ...
            if show:
                ...
                show_images(vis, titles = get_classes_with_index(attacker.labels), output_path=root, save_name='ori_grad_cam.png')
                
                show_images(attacker.images, titles = get_classes_with_index(attacker.labels), output_path=root, save_name='original_images.png')
                
                show_pixel_distribution(attacker.images, output_path=root, save_name='pixel_distribution.png')
                ...
                for mask_mode, parameters in mask_modes.items():
                    ...
                    for parameter in tqdm(parameters, desc="Parameters", leave=False):
                        ...
                        if parameter is None:
                            pixel_attacked, success_rate_dict, attack_ratio_per_channel, l1_norm, l2_norm_squre, original_loss, loss_dict_attacked, pred_loss_dict = attacker.attack(algo=algo, eta_list=eta_list, show=show, mask_mode=mask_mode)  
                        else:
                            pixel_attacked, success_rate_dict, attack_ratio_per_channel, l1_norm, l2_norm_squre, original_loss, loss_dict_attacked, pred_loss_dict = attacker.attack(algo=algo, eta_list=eta_list, show=show, mask_mode=mask_mode, **{mask_mode: parameter}) 
                        end_time = time.time()
